/**
 * Version:
 *     $Id: BiospecimensReport.java,v 1.1 2009/03/17 01:03:55 ray.welburn Exp $
 *
 * Revisions:
 *     $Log: BiospecimensReport.java,v $
...
public class BiospecimensReport extends DBBaseObject {
	public String[][] reportData = new String[19][2];

	public BiospecimensReport(Study study) throws Exception{
		super();
	...
	private void getReportData(Study study) throws Exception {
		...
		String strSQL = "SELECT `bt`.`name` AS `name`, count(`b`.`biotype_id`) AS `number` " +
				"FROM `biotypes` bt LEFT OUTER JOIN (SELECT * FROM biospecimens WHERE study_id = " + study.getId() + ") b " +
				"ON `bt`.`id` = `b`.`biotype_id` " +
				"GROUP BY `bt`.`id` " +
				"ORDER BY bt.id";
		try {
