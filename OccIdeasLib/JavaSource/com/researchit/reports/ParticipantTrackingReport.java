/**
 * Version:
 *     $Id: ParticipantTrackingReport.java,v 1.1 2009/03/17 01:03:55 ray.welburn Exp $
 *
 * Revisions:
 *     $Log: ParticipantTrackingReport.java,v $
...
public class ParticipantTrackingReport extends DBBaseObject {
	public String[][] reportData = new String[4][2];

	public ParticipantTrackingReport(Study study) throws Exception{
		super();
	...
	private void getReportData(Study study) throws Exception {
		...
		String strSQL = "SELECT `ps`.`name` AS `name`, count(`p`.`participantstatusid`) AS `number` " +
				...
				"WHERE ps.module = 'p'  GROUP BY `ps`.`id` " +
